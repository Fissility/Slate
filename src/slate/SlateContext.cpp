...
Object* SlateContext::processSyntaxLine(std::string line) {
	...
	std::vector<Lexer::ObjectSyntaxWrapper*> tokens;
	Lexer::lexer(line, definitions, tokens);

	Parser::ParserOutput output = Parser::parser(tokens);

	if (output.call == Parser::ContextCalls::RESULT) {
		std::vector<std::string> unknowns;
		Expression* expression = expressionFromAST(output.nodes[0], unknowns);
		return expression;
	}
